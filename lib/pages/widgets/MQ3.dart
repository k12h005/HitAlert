...
class _AlcoholPageState extends State<AlcoholPage> {
  ...
  void getData() async {
    ...
    http.Response response = await http.get(
        Uri.parse("https://hitalert.000webhostapp.com/API/MQ3_Featch.php"));

    if (response.statusCode == 200) {
      data = response.body;
      setState(() {
        isLoading = false;
        all_data = jsonDecode(data!)['MQ3'];
      });
    }
  }
                  ...
                  Padding(padding: EdgeInsets.only(left: 16,right: 16,top: 24,bottom: 8),
                    child: Row(
                      children: [
                        ...
                        Text(jsonDecode(data!)['MQ3'][index]['READ_TIME'],
                  ...
                  Padding(padding: EdgeInsets.only(left: 16,right: 16,top: 8,bottom: 24),
                    child: Row(
                      children: [
                        ...
                        Text(jsonDecode(data!)['MQ3'][index]['Mq3_val'],
                            style: TextStyle(color: Colors.white,fontWeight: FontWeight.normal))
                      ],
                    ),
                  ),
